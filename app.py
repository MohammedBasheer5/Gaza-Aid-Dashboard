...
# Build bg_css as a plain string (NOT f-string) to avoid {{ }} conflicts
if bg_b64:
    bg_css = (
        ...
        '.main{' +
        'background:linear-gradient(rgba(6,20,43,0.70),rgba(6,20,43,0.70)),' +
        'url("data:image/jpg;base64,' + bg_b64 + '");' +
        'background-size:cover;background-position:center;background-attachment:fixed;}' 
    )
else:
...
section[data-testid="stSidebar"] a { color: #BFE3FF !important; }
...
section[data-testid="stSidebar"] [data-baseweb="tag"] svg {
  color: #ffffff !important; fill: #ffffff !important; opacity: 0.95 !important;
}
section[data-testid="stSidebar"] [data-testid="stFileUploader"] {
  background: #FFFFFF !important; border-radius: 12px !important;
  padding: 10px !important; box-shadow: 0 6px 18px rgba(0,0,0,0.15) !important;
}
...
section[data-testid="stSidebar"] [data-testid="stFileUploader"] label span {
  color: #0B1F3A !important; -webkit-text-fill-color: #0B1F3A !important; font-weight: 700 !important;
}
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] {
  background: #EEF4FF !important; border: 1.5px dashed #3B8ED6 !important; border-radius: 10px !important;
}
...
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzoneInstructions"] small {
  color: #4A6080 !important; -webkit-text-fill-color: #4A6080 !important;
}
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] button {
  background: #FFFFFF !important; color: #0B1F3A !important;
  -webkit-text-fill-color: #0B1F3A !important; border: 1px solid #3B8ED6 !important;
  border-radius: 8px !important; font-weight: 700 !important;
}
/* ====== HEADER ====== */
.pro-title {
  text-align: center; font-weight: 950; font-size: 3.6rem !important;
...
.pro-sub {
  text-align: center; font-size: 1.55rem !important; font-weight: 600;
  margin: 0 0 8px 0; color: #B8D8F0 !important;
  -webkit-text-fill-color: #B8D8F0 !important; letter-spacing: 0.2px;
...
.pro-sub2 {
  ...
  text-shadow: 0 2px 18px rgba(0,0,0,0.90), 0 1px 4px rgba(0,0,0,1.0);
}
.pro-hr {
  border: 0; height: 1.5px;
  background: linear-gradient(90deg, transparent 0%, rgba(255,210,120,0.60) 20%,
    rgba(255,255,255,0.50) 50%, rgba(255,210,120,0.60) 80%, transparent 100%);
...
.badge {
  ...
  background: rgba(255,255,255,0.75); color: #0b2a52; box-shadow: var(--shadow2);
}
.badge.dark {
  background: rgba(7, 23, 51, 0.55); color: #EAF2FF;
...
.kpi {
  ...
  border-radius: 18px; padding: 14px 14px; box-shadow: var(--shadow); backdrop-filter: blur(4px);
}
...
/* ====== SECTIONS ====== */
.h-sec { color: #FFFFFF; font-weight: 950; margin: 6px 0 2px; text-shadow: 0 8px 22px rgba(0,0,0,0.25); }
.p-muted { color: rgba(255,255,255,0.80); margin: 0 0 10px; }
/* ====== TABS ====== */
[data-testid="stTabs"] {
...
[data-testid="stTabs"] [role="tablist"] {
  background: rgba(3,10,26,0.88) !important; border-radius: 14px !important;
  padding: 5px 6px !important; gap: 4px !important;
  border: 1px solid rgba(42,163,255,0.25) !important; flex-wrap: wrap !important;
  ...
  max-width: 100% !important; display: flex !important; overflow: visible !important;
}
button[data-baseweb="tab"] {
  ...
  padding: 9px 15px !important; border: 1px solid rgba(42,163,255,0.12) !important;
  transition: all 0.20s ease !important; white-space: nowrap !important;
  cursor: pointer !important; text-shadow: none !important;
}
button[data-baseweb="tab"]:hover {
  background: rgba(42,163,255,0.18) !important; color: #FFFFFF !important;
  -webkit-text-fill-color: #FFFFFF !important;
  border-color: rgba(42,163,255,0.55) !important; transform: translateY(-1px) !important;
...
button[data-baseweb="tab"][aria-selected="true"] {
  ...
  transform: translateY(-1px) !important; text-shadow: 0 1px 6px rgba(0,0,0,0.30) !important;
}
...
[data-testid="stMetricLabel"] span {
  color: #FFFFFF !important; -webkit-text-fill-color: #FFFFFF !important;
  font-weight: 700 !important; text-shadow: 0 2px 8px rgba(0,0,0,0.60) !important;
}
...
[data-testid="stMetricDelta"] { color: #7DD3FF !important; -webkit-text-fill-color: #7DD3FF !important; }
[data-testid="stMetric"] {
  background: rgba(5,17,45,0.72) !important; border-radius: 14px !important;
  padding: 14px 18px !important; border: 1px solid rgba(42,163,255,0.20) !important;
...
# Inject CSS (inject bg_css separately as plain string)
st.markdown(CSS.replace("BG_PLACEHOLDER", bg_css), unsafe_allow_html=True)
...
def load_and_clean(file_bytes=None, path=None):
    if file_bytes is not None:
        df = pd.read_excel(file_bytes); src = "Uploaded file"
    else:
        df = pd.read_excel(path); src = f"Local: {path}"
    ...
    df["Quantity"] = pd.to_numeric(df.get("Quantity", 0), errors="coerce").fillna(0) if "Quantity" in df.columns else 0
    ...
    df["Status"] = df["Status"].fillna("Unknown").astype(str) if "Status" in df.columns else "Unknown"
...
def build_agg(df: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    return (d.groupby([pd.Grouper(freq=freq), "Cargo Category", "Crossing"], dropna=False)
             .agg(trucks=("No. of Trucks","sum"), quantity=("Quantity","sum"))
             .reset_index().rename(columns={"Received Date":"ds"}).sort_values("ds"))

def make_series(agg, cats, metric_col, start, end, freq):
    d = agg[(agg["ds"] >= start) & (agg["ds"] <= end) & agg["Cargo Category"].isin(cats)].copy()
    return d.groupby("ds")[metric_col].sum().sort_index().asfreq(freq).fillna(0)

def hw_forecast(series, periods, freq):
    series = series.asfreq(freq).fillna(0)
    if len(series) < 8 or series.sum() <= 0:
        idx = pd.date_range(series.index.max() + pd.tseries.frequencies.to_offset(freq), periods=periods, freq=freq)
        return pd.Series([0.0]*periods, index=idx)
    if freq=="D": sp, ok = 7, len(series)>=14
    elif freq=="W": sp, ok = 4, len(series)>=12
    else: sp, ok = 12, len(series)>=24
    try:
        model = ExponentialSmoothing(series, trend="add",
            seasonal="add" if ok else None, seasonal_periods=sp if ok else None).fit(optimized=True)
...
st.markdown('<h4 class="pro-sub2">Humanitarian Flow &nbsp;&bull;&nbsp; Supply Gaps &nbsp;&bull;&nbsp; Forecasting</h4>', unsafe_allow_html=True)
...
required_per_period = st.sidebar.number_input(
    f"Required {metric} / {freq_label}", min_value=0.0,
    value=float(default_required), step=10.0, format="%.0f",
)
...
selected_categories = st.sidebar.multiselect("Cargo Categories", options=all_categories,
...
selected_crossings = st.sidebar.multiselect("Crossings", options=all_crossings,
...
with b2:
    st.markdown(
        f'<div style="text-align:right;"><span class="badge dark">Range: {start.date()} → {end.date()} • {freq_label} • {metric}</span></div>',
...
momentum = float(series.iloc[-1] - series.iloc[-5:-1].mean()) if len(series) >= 5 else 0.0
...
with tab2:
    ...
    if series_plot.empty:
        ...
        fig = px.line(pd.DataFrame({"ds": series_plot.index, "value": series_plot.values}),
                      x="ds", y="value", title=f"{metric} Over Time ({freq_label})")
...
with tab3:
    ...
    if comp.empty:
        ...
        fig = px.area(comp, x="ds", y=metric_col, color="Cargo Category",
...
with tab4:
    ...
    if not enough_data:
        ...
        fig.update_layout(height=520, margin=dict(l=20,r=20,t=50,b=20))
        st.plotly_chart(fig, use_container_width=True)
        out = fcst_df.copy(); out["required"] = required_per_period
...
with tab5:
    ...
    with a2:
        ...
        if enough_data:
            ...
            st.download_button("⬇️ Download Forecast Alerts CSV",
                               data=f.to_csv(index=False).encode("utf-8"),
                               file_name="forecast_gap_alerts.csv", mime="text/csv")
        else:
...
with tab6:
    ...
    st.markdown(
        f'<p style="color:#FFFFFF;font-size:16px;font-weight:600;text-shadow:0 2px 8px rgba(0,0,0,0.70);">' +
        f'From <b>{df["Received Date"].min().date()}</b> to <b>{df["Received Date"].max().date()}</b></p>',
        unsafe_allow_html=True)
    ...
    st.download_button("⬇️ Download Aggregated CSV",
                       data=agg_f.to_csv(index=False).encode("utf-8"),
                       file_name="aggregated_filtered.csv", mime="text/csv")
...
with tab7:
    ...
    if len(series) == 0:
        ...
        max_day = series.idxmax(); max_val = float(series.max())
        min_day = series.idxmin(); min_val = float(series.min())
        ...
        with i1:
            st.markdown(f"""<div class="kpi" style="background:rgba(255,255,255,0.88);">
  ...
  <div class="n" style="font-size:14px;">
    ...
    First shortage date: <b>{risk_days.iloc[0]["date"].date() if len(risk_days)>0 else "None"}</b>
                ...
                st.info("Forecast risk insights require more data.")
...
with tab8:
    ...
    crossing_coords = {
        "Erez": (31.559, 34.565), "Western Erez": (31.555, 34.560),
        "Kerem Shalom": (31.219, 34.284), "Rafah Crossing": (31.262, 34.247),
        "Gate 96": (31.250, 34.320), "Kissufim": (31.367, 34.403), "JLOTS": (31.520, 34.430),
    }
    dmap = df.copy()
    dmap = dmap[(dmap["Received Date"] >= pd.to_datetime(start)) & (dmap["Received Date"] <= pd.to_datetime(end))]
    if selected_categories: dmap = dmap[dmap["Cargo Category"].isin(selected_categories)]
    if selected_crossings: dmap = dmap[dmap["Crossing"].isin(selected_crossings)]
    value_col = "No. of Trucks" if metric == "Trucks" else "Quantity"
    cross_sum = dmap.groupby("Crossing", as_index=False)[value_col].sum().rename(columns={value_col:"value"})
    ...
    if cross_sum.empty:
        ...
        fig = px.density_mapbox(cross_sum, lat="lat", lon="lon", z="value", radius=35, zoom=9.2,
            center=dict(lat=31.42, lon=34.35), mapbox_style="open-street-map",
            hover_name="Crossing", hover_data={"value":True,"lat":False,"lon":False},
            title=f"Heatmap by Crossings — {metric}")
        fig_pts = px.scatter_mapbox(cross_sum, lat="lat", lon="lon", size="value", size_max=28, zoom=9.2,
            center=dict(lat=31.42,lon=34.35), mapbox_style="open-street-map",
            hover_name="Crossing", hover_data={"value":True})
        for tr in fig_pts.data: fig.add_trace(tr)
